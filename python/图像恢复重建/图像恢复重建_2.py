...
def restore_image(noise_img, size=4):
    ...
    # -------------实现图像恢复代码答题区域----------------------------
    # 在边缘补0
    red_img = np.pad(res_img[:, :, 0], (size//2, size//2),
                     'constant', constant_values=(0, 0))
    green_img = np.pad(res_img[:, :, 1], (size//2, size//2),
                       'constant', constant_values=(0, 0))
    blue_img = np.pad(res_img[:, :, 2], (size//2, size//2),
                      'constant', constant_values=(0, 0))

    def Re(i, j, size, img):
        points = []
        for i_temp in range(i-size//2, i+size//2+1):
            for j_temp in range(j-size//2, j+size//2+1):
                if img[i_temp, j_temp] != 0:
                    points.append(img[i_temp, j_temp])
        try:
            img[i, j] = sum(points) / len(points)
        except:
            img[i, j] = 0
    ...
    for i in range(res_img.shape[0]):
        for j in range(res_img.shape[1]):
            res_img[i, j, 0] = red_img[i+size//2, j+size//2]
            res_img[i, j, 1] = green_img[i+size//2, j+size//2]
            res_img[i, j, 2] = blue_img[i+size//2, j+size//2]

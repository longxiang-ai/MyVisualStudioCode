...
#     # -------------实现图像恢复代码答题区域----------------------------
#     def Re(i,j,size,res_img,color):
#         points = []
#         for i_temp in range(int(i-size/2), int(i+size/2+1)):
#             for j_temp in range(int(j-size/2), int(j+size/2+1)):
#                 if res_img[i_temp, j_temp, color] != 0:
#                     points.append(res_img[i_temp, j_temp, color])
#         res_img[i, j, color] = sum(points) / len(points)

#     for i in range(size//2, res_img.shape[0]-size//2):
#         for j in range(size//2, res_img.shape[1]-size//2):
#             Re(i,j,size,res_img,0)
#             Re(i,j,size,res_img,1)
#             Re(i,j,size,res_img,2)
...
#     x = np.reshape(res_img[:,0,0],newshape=(res_img[:,0,0].shape[0],1))
#     y = np.reshape(res_img[:,0,0],newshape=(res_img[:,0,0].shape[0],1))
#     lr = LinearRegression()
#     # 训练模型
#     lr.fit(x,y)
#     # 计算R平方
...
def restore_image(noise_img, size=3):
    ...
    # -------------实现图像恢复代码答题区域----------------------------
    color = 0
    def re(i,j,res_img,color):
        if res_img[i,j,color] == 0:
            scores = []
            for i_temp in range(i-1,i+1+1):
                for j_temp in range (j-1,j+1+1):
                    if res_img[i_temp,j_temp,color]!=0:
                        scores.append(res_img[i_temp,j_temp,color])
        res_img[i,j,color] = sum(scores) / len(scores)
    for i in range (1,res_img[:,0,color].shape):
        for j in range (1,res_img[0,:,color].shape):
            re(i,j,res_img,0)
            re(i,j,res_img,1)
            re(i,j,res_img,2)
    # ---------------------------------------------------------------

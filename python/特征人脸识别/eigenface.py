...
def spilt_data(nPerson, nPicture, data, label):
    ...
    # 划分训练集和测试集
    train = data[:nPerson, :nPicture, :, :].reshape(
        nPerson*nPicture, rows*cols)
    train_label = label[:nPerson, :nPicture].reshape(nPerson * nPicture)
    test = data[:nPerson, nPicture:, :, :].reshape(
        nPerson*(allPicture - nPicture), rows*cols)
    test_label = label[:nPerson, nPicture:].reshape(
        nPerson * (allPicture - nPicture))
...
def eigen_train(trainset, k=20):
    ...
    u, sigma, v = np.linalg.svd(trainset)
    # 构建特征人脸
    fea_vecs = v[:k, :]
    feature = fea_vecs / np.linalg.norm(fea_vecs, axis=0,keepdims = True)
...
def rep_face(image, avg_img, eigenface_vects, numComponents=0):
    ...
    ###################################################################################
    #                                                                                 #
    # test_face (10304,) 一行向量，去掉平均脸
    test_face = image - avg_img
    # eigenface_vects (k,10304) , rep (k,)
    representation = np.matmul(test_face, eigenface_vects[:numComponents, :].T)
    # 使用特征脸的数目等于传入的参数的数量
...
def recFace(representations, avg_img, eigenVectors, numComponents, sz=(112, 92)):
    ...
    rec_faces = np.matmul(
        representations, (np.linalg.pinv(eigenVectors[:numComponents, :].T)))
    face = rec_faces + avg_img
...
datapath = 'D:/课程文件/vs-code/python/特征人脸识别/ORL.npz'
...
plot_gallery(train_vectors, train_labels, n_row=5, n_col=5)
# 返回平均人脸、特征人脸、中心化人脸
avg_img, eigenface_vects, trainset_vects = eigen_train(
    train_vectors, num_eigenface)
...
plot_gallery(eigenfaces, eigenface_titles, n_row=5, n_col=5)
# 人脸识别模型
train_reps = []
...
for i in range(20, 200, 20):
    representations, numEigenFaces = rep_face(
        image, avg_img, eigenface_vects, i)
    face, name = recFace(representations, avg_img,
                         eigenface_vects, numEigenFaces)
    faces.append(face)
    names.append(name)

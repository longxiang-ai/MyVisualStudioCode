...
public:
    ...
    // 插入
    // bool Insert(ElementType &element, ElementType &key);
    bool Insert(ElementType &key);
    // 删除
    // bool Delete(ElementType &element, ElementType &key);
    bool Delete(ElementType &key);
    // 查找
    // bool Search(ElementType &element, ElementType &key);
...
// 查找一个值为element的元素，找到了返回true，否则返回false
// bool SkipList::Search(ElementType &element, ElementType &key)
...
// bool SkipList::Insert(ElementType &element, ElementType &key)
bool SkipList ::Insert(ElementType &key)
{
    // cout << "----------begin to insert " << element << " , " << key << "-----------" << endl;
    ...
    // 将节点移动到最底层的下一个节点
    pre = pre->next[0];
    // 如果当前已经存在了同样的值，则代表插入失败了，把当前节点的value存为value
    if (pre->key == key)
    {
...
// 删除节点
// bool SkipList::Delete(ElementType &element, ElementType &key)
bool SkipList ::Delete(ElementType &key)
{
    // cout << "----------begin to delete " << element << " , " << key << "-----------" << endl;
...
int main()
{
    ...
    SkipList s;
    // int a = 3;
    // cout << s.Search(a, a) << endl;
    // cout << s.Insert(a, a) << endl;
    // cout << s.Delete(a, a) << endl;
    for (int j = 0; j < 1; j++)
    {
